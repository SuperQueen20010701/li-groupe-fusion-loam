...
std::string pgKITTIformat, pgTUMformat, pgScansDirectory, pgSCDsDirectory;
...
void saveOptimizedVerticesTUMformat(gtsam::Values _estimates, std::string _filename)
{
    ...
    for(const auto& key_value: _estimates) {
        ...
        Eigen::Matrix3d R2;
        R2 <<  col1.x(), col2.x(), col3.x(),
              col1.y(), col2.y(), col3.y(),
              col1.z(), col2.z(), col3.z();
        Eigen::Quaterniond q(R2);

        stream  << keyframeTimes.at(key_value.key) << " "
                << t.x() << " " << t.y() << " " << t.z() << " "
...
std::optional<gtsam::Pose3> doICPVirtualRelative( int _loop_kf_idx, int _curr_kf_idx )
{
    ...
    if (icp.hasConverged() == false || icp.getFitnessScore() > loopFitnessScoreThreshold) {
        ROS_WARN_STREAM("[SC loop] ICP fitness test failed (" << icp.getFitnessScore() << " > " << loopFitnessScoreThreshold << "). Reject this SC loop.");
        return std::nullopt;
    } else {
        ROS_INFO_STREAM("[SC loop] ICP fitness test passed (" << icp.getFitnessScore() << " < " << loopFitnessScoreThreshold << "). Add this SC loop.");
    }
...
void process_pg()
{
    while(1)
    {
		while ( !odometryBuf.empty() && !fullResBuf.empty() )
        {
            ...
            if(fabs(timeLaserOdometry - timeLaser) > 0.1){
                mBuf.unlock();
                break;
            }
...
void performSCLoopClosure(void)
{
    ...
    if( SCclosestHistoryFrameID != -1 ) { 
        ...
        // ROS_WARN_STREAM("Loop detected! - between " << prev_node_idx << " and " << curr_node_idx);
...
void pubMap(void)
{
    ...
    mKF.unlock(); 
    // ROS_ERROR("map has %d points", laserCloudMapPGO->size());
